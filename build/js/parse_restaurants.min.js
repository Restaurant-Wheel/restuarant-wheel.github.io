...
function parseSubmissions(tsv){tsv=tsv.substring(1,tsv.length-1);tsv=tsv.replace(/(?:\\[r])/g,'');tsv=tsv.replace(/(?:\\[t])/g,'\t');tsv=tsv.replace(/(?:\\[n])/g,'\n');let submissions=Papa.parse(tsv);console.log('Parsed submissions:\n');console.log(submissions);submissions=submissions.data;const allRestaurants={};const dislikes={};const vetoes={};for(let i=1;i<submissions.length;i++){[,email,submittedRestaurants,dislike,veto,superlike]=submissions[i];if(dislike){if(dislike in dislikes){dislikes[dislike]+=1;}else{dislikes[dislike]=1;}}
